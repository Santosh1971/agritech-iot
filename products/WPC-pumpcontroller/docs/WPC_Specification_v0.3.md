...
- **Digital** (contact closed/open, active when closed): the boolean "pump status"/"no power" reads, sent in the wire protocol and, since 24 Sep 2026, surfaced in the app as **Water Flow** (IN1) and **Power** (IN4) — see §3.4.
...
### 3.4 IN1/IN4 as digital contacts: Water Flow and Power (added 24 Sep 2026)

Per dealer feedback (Kamta, via Avinash), on the assembled Pump Node PCB these two channels are wired as digital contacts, not just generic analog sensing, using the same "closed = active" convention as the Master's own level/No-Power inputs:
- **IN1 = Water Flow.** Closed (shorted to GND) = flow detected = the pump is confirmed actually running — feedback on the ON/OFF command issued, not just the relay's own state. Surfaced in the app as each pump's "Flow confirmed" / "No flow (yet)" indicator; the latter is highlighted only while the relay is commanded ON, since "no flow" is expected and unremarkable while OFF.
- **IN4 = No Power** (renamed **Power** in the app, shown as *present/OK* or *absent*). Same closed-to-GND-means-present convention as the Master's own No-Power input (J1 `NoPWR_CONN`, §2.2) — closed = power present, open = no power. The Master's own `noPower` status field had this **inverted** until 24 Sep 2026 (comment read "polarity TBD"); corrected, and a new `powerOk` field (true = present) added alongside it. See `WPC_LoRa_Protocol_v0.3.md` §10 for the wire-level detail.

**Firmware defect found and fixed in the same change:** `digitalRead()` on GPIO34-39 (IN1=36, IN4=35 — ESP32's input-only ADC1 pins) proved unreliable once `analogSetPinAttenuation()` had configured them for ADC use: confirmed on the bench (two Pump boards, both channels) that an open/pulled-high input, correctly read as saturated-high (~1.05-1.1V) by `analogRead`, read a stable, wrong LOW via `digitalRead` — 100% reproducible, not noise. The digital reads (wire protocol, `/info`, console `ADC`) now threshold the calibrated mV reading instead (`contactActive()`, 500mV), the same technique already used for the IN1/IN4 status LEDs (§4) — those LEDs were unaffected by this defect since they never used `digitalRead`. A show-Power/show-Water-Flow toggle was added to the app (Connection screen → Dashboard display) for an installation that doesn't have these two contacts wired.

**Disassociating a pump** (Master forgets a Pump Node so it stops polling it and the Pump Node is free to join elsewhere) already existed at the protocol level (`POST /forget` / MQTT `{"cmd":"forget"}`, §6/§9 of the protocol doc) and in the app's Assign screen; a matching option was added directly on the Status screen's pump row (which is where it was being looked for) and a console `FORGET <slot>` command was added for parity.

**Found and fixed the same day:** the Master-side forget alone wasn't enough — a Pump Node still had that Master's ID saved and silently rejoined (a new slot) after its next restart, since nothing had told *it* to stop. Added the missing Pump-side half: the Pump's own Provision screen now has a **"Forget this Master"** button (`POST /forget` on the Pump Node / console `FORGET`) that clears its saved Master ID, stops it sending `JOIN_REQUEST`, and forces its relay off immediately. Fully disassociating a pump now needs both actions — the Master's own unpair dialog says so.
