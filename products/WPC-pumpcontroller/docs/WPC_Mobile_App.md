...
- **Connection screen** (top-right button; shows `Local` or the active installation's name): choose Local/Cloud, add/select/remove installations (add by Master ID, or "Add the one I'm on" while connected to a Master's WiFi), see broker/Master connection state, and — while on the Master's own WiFi — set the farm WiFi the Master should use for internet. A timeout after saving the WiFi isn't necessarily a failure: the Master can drop its own access point for a moment while it joins the router. The **Farm WiFi** row (26 Sep 2026) now names why it isn't connected, when it isn't — `network not found` (confirm against **Scan for WiFi networks** on the same screen: if the name doesn't appear there, the Master's radio genuinely can't see it from where it's mounted, not a bug), `check the password`, or `lost the connection`.
...
- **Status screen** shows the firmware version and the Master's internet state — since 26 Sep 2026, "Internet: not connected" also names *why* (`network not found` / `wrong password?` / `lost connection`, from the new `wifi.state` field) instead of leaving it a guess, and in Cloud mode a status's age is shown (`Updated Xs/Xm/Xh ago`, orange past ~25 s) since a stale retained status was otherwise indistinguishable from a live one unless the broker's last-will had already fired. In Cloud mode an orange **"Master is offline — last known state"** banner still shows separately when the broker's last-will says the Master is down (the retained status can be hours old and must not look live).
