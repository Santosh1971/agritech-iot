...
Java_com_nbagri_flashertester_NativeFlasher_readMac(
...
/*
 * Erases the whole flash chip — including the NVS partition, which a
 * normal flash (bootloader+partitions+app only, see flash() below) never
 * touches. Ported from the original NB Agri Flasher app's admin-only
 * "Erase chip" (esp-serial-flasher already exposes esp_loader_flash_erase)
 * and made a standard part of every production flash here instead of an
 * opt-in extra — 2026-09-16 bench finding: WiFi/MQTT credentials written
 * during a unit's own test run were still present after reflashing it for
 * the next round, since NVS lives outside the three offsets flash() writes
 * and a partial flash never clears it. Returns an esp_loader_error_t value,
 * same convention as flash().
 */
JNIEXPORT jint JNICALL
Java_com_nbagri_flashertester_NativeFlasher_eraseChip(
    JNIEnv *env, jobject thiz,
    jobject transport)
{
    (void)thiz;
    ...
    /* Reset regardless of outcome, same reasoning as readMac()'s comment —
       ...
       (flash()'s own connect_with_stub() moments later). */
    esp_loader_reset_target(&loader);
    esp_loader_deinit(&loader);
    return (jint)err;
}
...
JNIEXPORT jint JNICALL
Java_com_nbagri_flashertester_NativeFlasher_flash(
    JNIEnv *env, jobject thiz,
    jobject transport,
